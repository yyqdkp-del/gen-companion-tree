...
// [cite: 7, 20] 核心技术栈：Vercel + Next.js + Supabase Realtime
...
export default function HydroApp() {
  ...
  useEffect(() => {
    const syncData = async () => {
      ...
      if (childData?.length) setChildren(prev => childData.map(d => ({ ...d, emoji: d.name === 'William' ? '👦🏻' : '👶🏻' })))
    }
    syncData()
  ...
  return (
    <main className="fixed inset-0 w-full h-full overflow-hidden select-none" style={{ background: 'linear-gradient(135deg, #A7D7D9 0%, #D9A7B4 100%)' }}>
      
      {/* 1. 纹理背景：0.1 不透明度巨型标题 (对角排版) */}
      <div className="absolute top-[15%] right-[-10%] text-[15vw] font-bold text-[#2C3E50] opacity-10 pointer-events-none tracking-tighter italic whitespace-nowrap">
        GEN COMPANION
      </div>
      ...
      <motion.div 
        ...
        className="absolute top-[5%] left-[5%] z-50 cursor-pointer active:scale-90 transition-transform"
      >
        <div className="relative">
          <motion.div 
            animate={{ boxShadow: ['0 0 15px rgba(212,169,106,0.3)', '0 0 35px rgba(212,169,106,0.5)', '0 0 15px rgba(212,169,106,0.3)'] }}
            transition={{ duration: 4, repeat: Infinity }}
            className="w-16 h-16 rounded-full bg-white/90 flex items-center justify-center border border-white/50 shadow-xl overflow-hidden"
          >
      ...
      <section className="absolute inset-0 z-20 pointer-events-none">
        ...
        <WaterDrop 
          ...
          top="45%" right="28%" color="rgba(212, 169, 106, 0.4)" delay={1.2} 
        />
        <WaterDrop 
          icon={<Heart size={18}/>} label={currentChild?.name} value={currentChild?.status === 'active' ? '活跃' : '休眠'} 
          top="60%" right="10%" color="rgba(232, 168, 154, 0.4)" delay={2.4} 
        />
        <WaterDrop 
          icon={<Trees size={18}/>} label="清迈天气" value="28°" 
          top="75%" right="22%" color="rgba(154, 183, 232, 0.4)" delay={3.6} 
        />
      </section>
      ...
      <footer className="fixed bottom-12 left-0 right-0 z-50 px-10 flex flex-col items-center">
        ...
        <div className="w-full max-w-sm h-16 bg-white/10 backdrop-blur-2xl border border-white/20 rounded-full flex items-center px-8 justify-between shadow-lg">
          ...
          <button className="flex items-center gap-3 opacity-20">
...
function WaterDrop({ icon, label, value, top, right, color, delay, alert = false }: any) {
  return (
    <motion.div
