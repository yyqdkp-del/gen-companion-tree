  ...
  return (
    <main className="fixed inset-0 w-full h-full overflow-hidden select-none font-sans" style={{ background: THEME.bg }}>
      ...
      <section className="absolute inset-0 z-20 pointer-events-none">
        ...
        />
        <LiquidDrop 
          icon={<Heart size={18}/>} label="状态" value={currentChild?.status === 'active' ? '活跃' : '休眠'} 
          top="58%" right="10%" color="rgba(232, 168, 154, 0.4)" delay={3} 
        />
        <LiquidDrop 
          icon={<Trees size={18}/>} label="天气" value="28°" 
          top="72%" right="20%" color="rgba(154, 183, 232, 0.4)" delay={4.5} 
        />
      </section>
...
function LiquidDrop({ icon, label, value, top, right, color, delay, alert = false }: any) {
  return (
    ...
    >
      <div 
        className="w-24 h-24 backdrop-blur-xl border border-white/30 liquid-drop flex flex-col items-center justify-center p-4 relative overflow-hidden group"
