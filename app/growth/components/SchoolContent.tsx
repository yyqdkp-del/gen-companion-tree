...
export default function SchoolContent() {
  ...
  const openCamera = () => {
    window.dispatchEvent(new CustomEvent('openCamera', { detail: { source: 'school' } }))
  }
  ...
  return (
    <section style={{ maxWidth: 560, margin: '0 auto', width: '100%' }}>
      ...
      <p style={{
        ...
        适合处理学校邮件截图、纸质通知、校历图片、缴费单和活动单。点击底部右侧相机按钮拍照或上传，识别完成后会自动进入日安待办、校历和解析历史。
      </p>
        ...
        {[
          ...
          <div key={item.title} style={{
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: 13, lineHeight: 1.7, color: T.textDim, fontFamily: 'sans-serif' }}>{item.desc}</p>
            </div>
          </div>
        ))}
      </div>
